...
import CloseAvaxLongForm from "./CloseAvaxLongForm";
...
function PositionsTable() {
  ...
  const {
    ...
    onOpen: onDecreaseOpen,
    onClose: onDecreaseClose,
  } = useDisclosure();
  ...
  const getTotalSize = () => {
    return Object.values(positionsData)
      .flat()
      .reduce((acc, position) => acc + parseFloat(position.size), 0);
  };

  const getTotalCollateral = () => {
    return Object.values(positionsData)
      .flat()
      .reduce((acc, position) => acc + parseFloat(position.collateral), 0);
  };
  ...
  const renderPositionRows = (positionsArray) => {
    return positionsArray.map((position, index) => (
      <Tr key={index}>
        ...
        <Td><div>
            {parseInt(position.hasProfit) === 0 ? "-" : "+"}{" "}
            {position.deltaPercentage}
          </div></Td>
          <Td color={parseInt(position.hasProfit) === 0 ? 'red.500' : 'green.500'}>
        {position.delta}
      </Td>
  ...
  return (
    <Container maxW="container.lg" mt="3" bg="whitesmoke" color="darkblue">
        ...
        <ModalContent>
          ...
          <ModalBody>
            <ActionTabs
              ...
              sizeWei={activePosition ? activePosition.sizeWei : ""}
      ...
      <Box mt="4">
        ...
        <Text>Total Collateral: ${getTotalCollateral().toFixed(2)}</Text>
